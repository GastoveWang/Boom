...
def locate_event(
    ...
    # The original mine projection is calibrated for a 2840 x 2840 image.
...
def _draw_text(
    ...
    drawer.text(position, text, font=_get_font(size), fill=(color[2], color[1], color[0]))
...
def _draw_event_card(
    ...
    _draw_text(panel, f"時間  {event.timestamp_sec:.2f} 秒", (left + 18, top + 58), 21, (70, 82, 101))

    if not event.has_coordinate:
        _draw_text(panel, "座標無法取得", (left + 18, top + 103), 25, (40, 52, 72))
        _draw_text(panel, "請提供 DJI 定位參考照片", (left + 18, top + 143), 20, (88, 100, 119))
        return bottom + 14
