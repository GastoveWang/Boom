"""
==============================================================================
Boom 核心幾何數學 - TWD97 / WGS84 大地座標與地平投影運算 (coordinates.py)
==============================================================================

【檔案定位】
本檔案為純數學幾何轉換模組，不依賴任何外部執行器。提供靜態相機姿態投影、
WGS84 經緯度與台灣二度分帶 TWD97 大地平面座標系之間的互相轉換。
==============================================================================
"""
...
def wgs84_to_twd97(lon: float, lat: float) -> Tuple[float, float]:
    """將 WGS84 經緯度轉換為台灣 TWD97 二度分帶座標 (easting, northing)"""
    a = 6378137.0
    b = 6356752.3142451
    ...
    lon_rad = (lon - math.floor((lon + 180) / 360) * 360) * math.pi / 180
    lat_rad = lat * math.pi / 180
    v = a / math.sqrt(1 - e * math.sin(lat_rad) ** 2)
    t = math.tan(lat_rad) ** 2
    c = e2 * math.cos(lat_rad) ** 2
    a_term = math.cos(lat_rad) * (lon_rad - lon0)
    m = a * (
        (1 - e / 4 - 3 * e**2 / 64 - 5 * e**3 / 256) * lat_rad
...
def coordinate(
    ...
    twd_e, twd_n = wgs84_to_twd97(lon, lat)
