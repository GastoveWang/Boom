"""
==============================================================================
Boom 煙霧偵測演算法 - 光流與 PIDNet 兩階段融合偵測器 (optical_pidnet_fusion_detector.py)
==============================================================================

【檔案定位】
本檔案實作兩階段（Staged Fusion）荒野煙霧偵測策略。結合「光流演算法之極低延遲動態觸發」
與「PIDNet 深度學習語意分割之高精度特徵確認」，在保持快速反應的同時將誤報率降至最低。

【核心融合機制】
1. 階段一（光流候選觸發）：利用 `InstantSmokeDustDetector` 監控突然出現的運動擴散，
   記錄候選起煙影格、時間與初始像素範圍。
2. 階段二（PIDNet 特徵確認）：在融合時間視窗 (`fusion_window_sec`) 與距離半徑
   (`fusion_match_distance_px`) 內，若 PIDNet 亦回傳對應區域之煙霧確認事件，
   則正式確認為有效事件，並保留光流最初的起煙時間點以提供更精準的起煙時間估算。

【核心類別清單】
- `OpticalPIDNetFusionConfig`: 融合時間視窗與空間匹配半徑超參數設定。
- `OpticalPIDNetFusionDetector`: 協調整合兩大偵測器之融合偵測實作。
==============================================================================
"""
...
from boom.detection.optical_flow import (
    DetectorConfig,
    InstantSmokeDustDetector,
    OpticalCandidateEvent,
)
from boom.detection.pidnet import (
...
class OpticalPIDNetFusionDetector(BaseSmokeDetector):
