...
def read_dataset(source, min_samples=2, classes_file=None):
    if classes_file is None and not (source / "classes.txt").is_file() and (source / "data.yaml").is_file():
        config = yaml.safe_load((source / "data.yaml").read_text(encoding="utf-8-sig"))
        c_names = config.get("names", [])
        names = dict(enumerate(c_names)) if isinstance(c_names, list) else {int(k): v for k, v in c_names.items()}
    else:
