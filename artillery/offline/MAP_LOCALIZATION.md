...
指定 `--map-dir asset/maps` 時，程式先依子資料夾建立地圖範圍索引，再比對照片 GPS
及由高度／雲台姿態估計的畫面中心地面位置，自動選擇候選地區。
例如目前 0603、0604 的 `DJI_001_P.JPG`，飛機 GPS 都在地圖覆蓋範圍外，
但估計畫面中心落在「北岸」，因此載入北岸地圖。UI 顯示「候選地圖：北岸」。

區域重疊或 GPS／視線分屬不同區域時保留多個候選；都未命中時保留所有地圖，
標示區域未定，不強制套用最近的地圖。區域名稱取自資料夾，並非行政區反查。
判斷使用各 TIFF 矩形範圍；透明區域仍由載入後的有效遮罩檢查。
這只是在選擇地圖搜尋範圍，不代表地圖視覺匹配已成功。

1. 讀 GPS 照片 EXIF 與 DJI XMP，建立 GPS 起點、近似內參與地面平面。
2. 依 WGS84 GeoTIFF 地理標籤合成地圖，保留 Alpha 有效區域，避免透明邊界污染匹配。
   目前支援 north-up、EPSG:4326、PixelIsArea 的 `.tif`；其他形式會明確報錯。
3. 依照片姿態把照片投到地圖平面，SIFT／RANSAC 匹配，再以 PnP 驗證位置與重投影誤差。
...
- `reference_ground_projection.jpg`：參考照片依初始化姿態投影的診斷圖。
...
建立快取鍵。`--map-mpp` 控制地圖預覽解析度，預設 0.75 公尺／像素；首次載入較慢。
...
本次 0603 資料的照片對地圖 SIFT 匹配未通過品質門檻。照片對影片匹配可用，
